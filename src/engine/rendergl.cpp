...
VARP(zoomfov, 10, 45, 150);
VARP(fov, 10, 90, 150);
VARP(avatarzoomfov, 10, 30, 150);
VARP(avatarfov, 10, 65, 150);
...
VARFP(minimapsize, 7, 10, 11, { if(minimaptex) drawminimap(); });
...
void drawcrosshair(int w, int h)
{
    ...
    { 
        ...
        if(index == 1)
        {
            ...
            // so the player can still know where they're aiming and know their health status
    ...
    hudshader->set();
    if(game::selectcrosshair(color) == 1)
    {
        // use blue color for teammate crosshair
        gle::color(vec(0.1, 0.4, 1));
    }
    else
    {
    gle::color(color);
